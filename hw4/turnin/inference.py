...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for posi in self.legalPositions:
                dist = util.manhattanDistance(posi, pacmanPosition)
                allPossible[posi] = emissionModel[dist] * self.beliefs[posi]
        "*** END YOUR CODE HERE ***"
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for old in self.legalPositions:
            ...
            for new, prob in newPosDist.items():
                allPossible[new] += prob * self.beliefs[old]
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        posiLen = len(self.legalPositions)

        self.particles = self.legalPositions * (self.numParticles / posiLen + 1)
    ...
    def observe(self, observation, gameState):
        ...
        allPossible, belief = util.Counter(), self.getBeliefDistribution()
        for loc in self.legalPositions:
            dis = util.manhattanDistance(loc, pacmanPosition)
            allPossible[loc] += emissionModel[dis] * belief[loc]
        # 2)
        if not any(allPossible.values()):
            self.initializeUniformly(gameState)
            return
        self.particles = [util.sample(allPossible) for _ in xrange(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.particles = [util.sample(self.getPositionDistribution(self.setGhostPosition(gameState, old)))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        posi = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
        posiLen = len(posi)

        random.shuffle(posi)
        self.particles = posi * (self.numParticles / posiLen + 1)
        self.particles = self.particles[:self.numParticles]
        ...
        for part in self.particles:
            ...
            for i in xrange(self.numGhosts):
                if noisyDistances[i] is None:
                    ...
                    dist = util.manhattanDistance(part[i], pacmanPosition)
                    val *= emissionModels[i][dist]
            allPossible[part] += val
